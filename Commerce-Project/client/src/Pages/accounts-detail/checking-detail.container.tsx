...
const CheckingDetail = () => {
  ...
  const getBalances = async () => {
    ...
    const format = check.data.data[0].amount;
    setChecking(numberWithCommas(format));
  }
  ...
  const addNewTransaction = () => {
    ...
    apis.createChecking(form, username)
      ...
      <Paper elevation={5} className={gridMargin}>
        <Grid rows={rows} columns={columns}>
          ...
          <Toolbar variant='dense'>
            ...
            <Typography className={balance}>{checking}</Typography>
            ...
            {rows.length > 0 ?
              <CSVLink
                ...
                data={rows}
                filename={"Checking-Transactions.csv"}
                uFEFF={false}
                target='_blank'
              >
        ...
        <DialogContent>
          <Layout container justify='center' className={layoutMargin}>
            <Layout container justify='space-around'>
              <TextField label='Account' value='Checking' disabled />
              <FormGroup>
                <FormLabel>
                  Amount
                  </FormLabel>
